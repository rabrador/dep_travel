...
import com.example.han.newtravel30.Adapter.ListUsersAdapter;
...
public class ListUsersActivity extends AppCompatActivity {
        ...
        btnCreateChat.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (lstUsers.getCheckedItemPositions().size() == 1)
                    createPrivteChat(lstUsers.getCheckedItemPositions());
        ...
        QBRestChatService.createChatDialog(dialog).performAsync(new QBEntityCallback<QBChatDialog>() {
            ...
            public void onSuccess(QBChatDialog qbChatDialog, Bundle bundle) {
                ...
                for (int i = 0; i < qbChatDialog.getOccupants().size(); i++) {
                    qbChatMessage.setRecipientId(qbChatDialog.getOccupants().get(i)); //群組使用者
                    try {
    ...
    private void createPrivteChat(SparseBooleanArray checkedItemPositions) {
        final ProgressDialog mDialog = new ProgressDialog(ListUsersActivity.this);
        mDialog.setMessage("Please waiting...");
        mDialog.setCanceledOnTouchOutside(false);
        mDialog.show();
            ...
            if (checkedItemPositions.get(i)) {
                ...
                QBRestChatService.createChatDialog(dialog).performAsync(new QBEntityCallback<QBChatDialog>() {
                    ...
                    public void onSuccess(QBChatDialog qbChatDialog, Bundle bundle) {
                        ...
                        QBSystemMessagesManager qbSystemMessagesManager = QBChatService.getInstance().getSystemMessagesManager();
                        QBChatMessage qbChatMessage = new QBChatMessage();
                        qbChatMessage.setRecipientId(user.getId()); //取得接收訊息者ID
                        qbChatMessage.setBody(qbChatDialog.getDialogId());
                        try {
                            qbSystemMessagesManager.sendSystemMessage(qbChatMessage);
                        } catch (SmackException.NotConnectedException e) {
